/**
 * ROLEPLAY AND GL FILLER (FIXED APPEND BUGS)
 * We collect all characters into a single massive array then do ONE delete and insert.
 */
...
const GL_CHARS = [
    ...
    { n: "Rae Taylor", s: "Villainess" }, { n: "Claire François", s: "Villainess" },
    { n: "Utena Tenjou", s: "Utena" }, { n: "Anthy Himemiya", s: "Utena" },
    ...
    { n: "Chitose Kuroda", s: "Yuri is My Job!" }, { n: "Anisphia", s: "Magical Rev" },
    { n: "Euphyllia", s: "Magical Rev" }
];
...
async function fillRest(tag, startIdx, roleNameFn, targetCount=100) {
    let results = [];
    const query = `query{Page(page:2,perPage:${targetCount}){characters(sort:FAVOURITES_DESC){id name{full} image{large}}}}`;
    try {
        const res = await fetch("https://graphql.anilist.co", {
            method: "POST", headers: { "Content-Type": "application/json" },
...
async function seedDB(tag, chars) {
    ...
    let n = 0;
    try {
        await client.query("DELETE FROM characters WHERE tag=$1", [tag]); // ONLY DO IT HERE ONCE
        for (let i = 0; i < chars.length; i++) {
            const c = chars[i];
            const id = `${tag.toLowerCase()}-f3-${Date.now()}-${i}`;
...
async function main() {
    console.log("=== ROLEPLAY & GL SINGLE BULK INSERT ===\n");

    // 1. GET PREVIOUS BL/GAME TO PRESERVE THEM (if script accidentally targets them)
    // We are only touching Roleplay and GL here.

    console.log("[Roleplay] Building Master List...");
    let rpResults = [];
    ...
    const rpRest = await fillRest("Roleplay", rpResults.length, (i) => genericRoles[i % genericRoles.length], 100 - rpResults.length);
    rpResults.push(...rpRest);
    if(rpResults.length > 0) await seedDB("Roleplay", rpResults);

    console.log("\n[GL] Building Master List...");
    let glResults = [];
    for(let i=0; i<30; i++) {
        const m = GL_CHARS[i];
        if(!m) break;
    ...
    const glRest = await fillRest("GL", glResults.length, (i) => `GL Character ${i}`, 100 - glResults.length);
